...
void main() {
    ...
    'bagian': [
      {
        ...
        'matriks': {
          ...
          'baris': [
            ...
            {'kode': 'indikator_pressure', 'label': 'Indikator Pressure', 'tipe': 'indikator_tekanan', 'satuan': 'bar', 'kode_data': 'tekanan.indikator'},
          ],
        },
      },
    ],
  ...
  LembarKerjaState isianBaru() =>
      LembarKerjaState(bentuk: LembarKerja.fromJson(bentukJson()), clientRequestId: 'uji-foto-matriks');
  ...
  List<TeksTerbaca> fotoMatriks({Set<String> hilang = const {}}) {
    ...
    for (var b = 0; b < m.semuaBaris.length; b++) {
      ...
      for (var k = 0; k < m.titikWaktu.length; k++) {
        hasil.add(
          kata(b == 0 ? jam(k) : bacaan(b - 1, k), xKolom(k), y),
        );
      }
    }
  ...
  test('bacaan manometer nggak pernah mendarat di kolom suhu', () {
    ...
    final terisi = isian.terapkanHasilFotoMatriks(m, petakan(m, fotoMatriks()).sel);
    ...
    for (var b = 0; b < m.baris.length; b++) {
      for (var k = 0; k < m.titikWaktu.length; k++) {
        expect(
          ...
          bacaan(b, k),
          reason: '${m.baris[b].label} titik waktu ${m.titikWaktu[k]} salah isi',
        );
      }
    }
  });
  ...
  test('baris yang namanya nggak kebaca nggak keisi, dan nggak nyedot tetangga', () {
    final m = matriks();
    final isian = isianBaru();
    addTearDown(isian.dispose);

    final hasil = petakan(m, fotoMatriks(hilang: const {'Indikator Pressure'}));
