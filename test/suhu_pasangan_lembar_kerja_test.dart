...
void main() {
  ...
  group('bentuk dari server kebaca', () {
    test('ketiganya berpasangan, dan tiap tabel punya peran + grup sendiri', () {
      for (final kode in ['thermocouple', 'thermometer_glass', 'thermohygro']) {
        final b = bentuk(kode);
        final tabel = tabelDari(b);

        expect(b.berpasangan, isTrue, reason: '$kode harusnya lembar pasangan');
    ...
    test('Thermocouple & Gelas bukan lembar dua-parameter', () {
      for (final kode in ['thermocouple', 'thermometer_glass']) {
        ...
        expect(tabel.every((t) => t.chamberPerBaris.isEmpty), isTrue, reason: kode);
      }
    });
  });
  ...
  group('kunci baris', () {
    test('deret standar & UUT berbagi kunci yang SAMA', () {
      ...
      for (var i = 0; i < state.barisTabel(standar).length; i++) {
        expect(
          ...
          reason: 'Dua deret itu SET POINT YANG SAMA, dibaca bergantian — '
    ...
    test('set point 50 di dua blok Thermohygro nggak saling menelan', () {
      ...
      // Ini penjaga nomor 2.
      expect(
        state.titik, hasLength(10),
        reason: 'Set point 50 ada di dua blok (50 °C & 50 %RH). Dikunci ke '
  ...
  group('payload', () {
    test('dua deret sampai utuh, sel kosong tetap di posisinya', () {
      ...
      for (var i = 0; i < 5; i++) {
        titik.kotak('standar', 'pembacaan', i).text = '49.5';
      }
      // Repeat 2 UUT sengaja DIKOSONGKAN — nomor pengulangan sesudahnya nggak
      // boleh geser.
      for (var i = 0; i < 5; i++) {
        if (i == 1) continue;
        titik.kotak('uut', 'pembacaan', i).text = '49.9';
      }
      ...
      expect(
        ...
        reason: 'Sel kosong tetap null DI POSISINYA. Dibuang, Repeat 3 naik '
    ...
    test('titik yang belum disentuh nggak ikut dikirim', () {
      ...
      final tabel = tabelDari(b);
      final standar = tabel.firstWhere((t) => t.deretStandar);
      final titik = state.titik[state.kunciBaris(state.barisTabel(standar), 0, standar)]!;
      titik.kotak('standar', 'pembacaan', 0).text = '29.9';

      expect(titik.pasanganKosong, isFalse);
      expect(
        state.titik.values.where((t) => !t.pasanganKosong),
        hasLength(1),
        reason: 'Baris kosong di tengah bikin `titik_ke` sesudahnya geser di '
    ...
    test('parameter ikut tiap titik Thermohygro', () {
      ...
      for (final t in state.titik.values) {
        for (final peran in ['standar', 'uut']) {
          t.kotak(peran, 'pembacaan', 0).text = '1';
        }
      }
      ...
      expect(
        payload.where((m) => m['parameter'] == 'suhu'),
        hasLength(5),
      );
      expect(
  ...
  group('sesi yang dikembalikan admin pulang utuh', () {
    ...
    test('pembacaan pulih ke deret yang benar, bukan saling menimpa', () {
      ...
      expect(
        ...
        '49.9',
        reason: 'Deret UUT punya kotaknya SENDIRI — kalau dua deret berbagi '
    ...
    test('50 °C dan 50 %RH Thermohygro pulih ke blok masing-masing', () {
      ...
      final suhu = state.titik.values
          .firstWhere((t) => t.titikUkur == 50 && t.satuan == '°C');
      final rh = state.titik.values
          .firstWhere((t) => t.titikUkur == 50 && t.satuan == '%RH');

      expect(suhu.kotak('standar', 'pembacaan', 0).text, '54.91');
      expect(
        rh.kotak('standar', 'pembacaan', 0).text,
        '49.4',
        reason: 'Tanpa satuan sebagai pembeda, angka blok kelembapan mendarat '
    ...
    test('sesi lama tanpa kolom satuan tetap pulih, nggak dibuang', () {
      ...
      final kebuang = state.terapkanPembacaan([
        mentah(titikUkur: 50, pembacaanKe: 1, pembacaan: 49.5, peran: 'standar'),
      ]);

      expect(
        kebuang,
        0,
        reason: 'Angka yang mendarat di baris yang benar tapi satuannya nggak '
  ...
  group('peran pasangan dipisah dari peran GRID', () {
    test('standar/uut bukan baris grid Enclosure', () {
      ...
      expect(
        pasangan.bagianGrid,
        isFalse,
        reason: 'Dua-duanya memakai kolom `peran_sensor` yang sama. Nggak '
