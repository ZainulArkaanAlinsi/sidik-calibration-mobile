...
> - **`nomor_order`** (opsional, string) & **`tanggal_terima`** (opsional, tanggal, ≤ `tanggal_kalibrasi`) — ✅ **Live sejak 20 Jul.** Field baru di level sesi (bukan per titik). `nomor_order` murni catatan bebas (mis. `"2405.13.A"`). Balik di response sebagai `nomor_order` & `tanggal_terima` (ISO 8601). Mobile sekarang punya field-nya di `ph_calibration_input_screen.dart` (Nomor Order + date picker Tanggal Terima, di atas Tanggal Kalibrasi).
> - **`titik[].metode`** — ✅ **Live sejak 20 Jul.** Nomor & revisi IK yang dipakai titik ini (mis. `"SIDIK-IK-CAL-0506_Rev.6"`), diambil dari `CalibrationCapability.metode` waktu titik itu matched ke CMC. `null` kalau titik lewat jalur generik (nggak ada CMC yang cocok).
