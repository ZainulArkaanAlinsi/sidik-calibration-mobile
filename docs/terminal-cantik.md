...
| Prompt berubah di PowerShell biasa, tapi **tidak di terminal VS Code** | Tiga sebab, urut dari yang paling sering: (1) terminalnya sudah kebuka sebelum dipasang; (2) kamu masih pakai skrip versi lama yang menulis ke profil per-host (`Microsoft.PowerShell_profile.ps1`) — VS Code membaca `Microsoft.VSCode_profile.ps1`; (3) **edisi PowerShell-nya beda** — Windows PowerShell 5.1 dan PowerShell 7 punya `profile.ps1` masing-masing (`Documents\WindowsPowerShell\` vs `Documents\PowerShell\`), jadi memasang dari jendela 5.1 tidak kebaca di VS Code yang pakai pwsh 7 | `git pull` lalu jalankan skripnya lagi (versi sekarang menulis ke `profile.ps1` yang dibaca semua host **pada edisi itu**), **tutup terminal VS Code-nya**, buka yang baru. Masih bawaan juga? Berarti sebab (3): jalankan skripnya sekali lagi **dari dalam terminal VS Code itu**. Skripnya mencetak edisi mana yang barusan dipasang |
